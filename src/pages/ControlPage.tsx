  ...
  return (
    <div style={{ padding: "66px 20px 32px", maxWidth: 1280, margin: "0 auto" }}>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 24 }}>
          <div>
            <OrbitronText size={11} color={C.textLo} style={{ letterSpacing: "0.3em", fontSize: 10, display: "block", marginBottom: 6, fontWeight: 200 }}>
              INTELIGÊNCIA ORBITAL AUTÓNOMA — ANO 2084
            </OrbitronText>
            <OrbitronText size={42} color={C.blue}>PROJECT NEBULA</OrbitronText>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontFamily: "'Share Tech Mono',monospace", fontSize: 9, color: C.textLo, marginBottom: 4 }}>TEMPO DE MISSÃO DECORRIDO</div>
            <OrbitronText size={22} color={C.cyan}>847:12:33</OrbitronText>
          </div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(170px,1fr))", gap: 12, marginBottom: 20 }}>
          {[
            { l: "SATÉLITES ATIVOS", v: `${nominal}/5`,             c: C.green },
            { l: "DETRITOS RASTREADOS",    v: "247",                       c: C.blue },
            { l: "RISCO DE COLISÃO",    v: `${critRisk?.risk ?? 0}%`,   c: critRisk ? C.red : C.green },
            { l: "COBERTURA ORBITAL",  v: "94.7%",                     c: C.cyan },
            { l: "CORREÇÕES DE IA",    v: "1,284",                     c: C.purple2 },
            { l: "ESTADO DO UPLINK",     v: "EXCELENTE",                   c: C.green },
          ].map(m => (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 16, marginBottom: 16 }}>
          <Panel glow style={{ height: 400 }}>
            <div style={{ padding: "14px 18px 0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <SLabel>VISUALIZAÇÃO ORBITAL</SLabel>
              <Tag>AO VIVO</Tag>
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <Panel>
              <PanelInner>
                <SLabel>ESTADO DA CONSTELAÇÃO</SLabel>
                {sats.map((sat: any) => (
                  <div key={sat.id} onClick={() => onSelectSat(sat)}
                    ...
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 3 }}>
                      ...
                      <div style={{ fontFamily: "'Share Tech Mono',monospace", fontSize: 8, color: C.textLo }}>SIN {sat.signal}%</div>
                    </div>
                  </div>
                ))}
              </PanelInner>
            ...
            <Panel>
              <PanelInner>
                <SLabel color={C.red}>ALERTAS DE PROXIMIDADE DE DETRITOS</SLabel>
                  ...
                  <div key={d.id} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        ...
                        <span style={{ fontFamily: "'Exo 2',sans-serif", fontSize: 12, fontWeight: 700, color: riskColor(d.risk) }}>{d.risk}% RISCO</span>
                      </div>
        ...
        <Panel>
          <PanelInner>
            <SLabel>TELEMETRIA DA FROTA — TEMPO REAL</SLabel>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 16 }}>
              <TelemetryChart data={telHistory} dataKey="battery"    color={C.green}   label="MÉDIA DA BATERIA" />
              <TelemetryChart data={telHistory} dataKey="signal"     color={C.blue}    label="FORÇA DO SINAL" />
              <TelemetryChart data={telHistory} dataKey="propulsion" color={C.purple2} label="PROPULSÃO" />
              <TelemetryChart data={telHistory} dataKey="coverage"   color={C.cyan}    label="COBERTURA ORBITAL" />
            </div>
          </PanelInner>
        </Panel>
      </motion.div>
